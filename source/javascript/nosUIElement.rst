...
.. js:function:: $.nosUIElement(element [, data ])
	...
	:param JSON element: JSON definition of the element to create.
		...
		:type: ``string``. ``button`` (default) or ``link``. See :js:fun:`$context.nosFormUI` for buttons ``data``, those of links are almost the same.
		:label: ``string``. Element label.
		:action: ``{}``. Action bound to click event. See :js:func:`$context.nosAction`.
		...
		:menu: ``{}``. If set, bind a context menu to element.
			...
			:menus: ``[{}]``. Array containing each menu line.
				...
				:action: ``{}``. Action bound to click event of menu line. See :js:func:`$context.nosAction`.
	...
	:param JSON data: Data attached to element and passed to action. See :js:func:`$context.nosAction`.
